...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">测试管理</li>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" id="xiangmuCeshiNameSearch" style="width: 140px;" class="form-control form-control-sm"
                                                   placeholder="模块名称" aria-controls="tableId">
                                        </div>
                                    </div>
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        模块状态
                                        <div class="layui-input-inline">
                                            <select name="xiangmuCeshiTypesSelectSearch"  style="width: 150px;" id="xiangmuCeshiTypesSelectSearch" class="form-control form-control-sm"
    ...
    <script>
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //模块名称
            var xiangmuCeshiNameSearchInput = $('#xiangmuCeshiNameSearch');
            if( xiangmuCeshiNameSearchInput != null){
                if (xiangmuCeshiNameSearchInput.val() != null && xiangmuCeshiNameSearchInput.val() != '') {
                    searchForm.xiangmuCeshiName = $('#xiangmuCeshiNameSearch').val();
                }
            }
     
                //模块状态
            var xiangmuCeshiTypesSelectSearchInput = document.getElementById("xiangmuCeshiTypesSelectSearch");
            if(xiangmuCeshiTypesSelectSearchInput != null){
                var xiangmuCeshiTypesIndex = xiangmuCeshiTypesSelectSearchInput.selectedIndex;
                if( xiangmuCeshiTypesIndex  != 0){
                    searchForm.xiangmuCeshiTypes= document.getElementById("xiangmuCeshiTypesSelectSearch").options[xiangmuCeshiTypesIndex].value;
                }
            }
                        getDataList();
        }
        ...
        function getDataList() {
            http("xiangmuCeshi/page", "GET", {
                page: pageIndex,
                limit: pageSize,
                sort: sortColumn,
                order: sortOrder,
                //本表的
                xiangmuCeshiName: searchForm.xiangmuCeshiName,
                xiangmuCeshiTypes: searchForm.xiangmuCeshiTypes,
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //模块名称
            var xiangmuCeshiNameCell = document.createElement('td');
            xiangmuCeshiNameCell.innerHTML = item.xiangmuCeshiName;
            row.appendChild(xiangmuCeshiNameCell);


            //模块状态
            var xiangmuCeshiTypesCell = document.createElement('td');
            xiangmuCeshiTypesCell.innerHTML = item.xiangmuCeshiValue;
            row.appendChild(xiangmuCeshiTypesCell);


                //测试详情
            var xiangmuCeshiContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.xiangmuCeshiContent && item.xiangmuCeshiContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.xiangmuCeshiContent.indexOf('<img');
                var endIndex = item.xiangmuCeshiContent.indexOf('>');
                    xiangmuCeshiContentCell.innerHTML = item.xiangmuCeshiContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.xiangmuCeshiContent != null && item.xiangmuCeshiContent != "" && item.xiangmuCeshiContent.length >= 11) {
                        xiangmuCeshiContentCell.innerHTML = item.xiangmuCeshiContent.substring(0, 10) + "...";
                } else {
                        xiangmuCeshiContentCell.innerHTML = item.xiangmuCeshiContent;
                }
            }
            row.appendChild(xiangmuCeshiContentCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("xiangmuCeshi/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addxiangmuCeshi", "addxiangmuCeshi");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //填充本表搜索下拉框
        ...
        function xiangmuCeshiTypesSelectSearch() {
            var xiangmuCeshiTypesSelectSearch = document.getElementById('xiangmuCeshiTypesSelectSearch');
            if(xiangmuCeshiTypesSelectSearch != null) {
                xiangmuCeshiTypesSelectSearch.add(new Option('-请选择-',''));
                if (xiangmuCeshiTypesOptions != null && xiangmuCeshiTypesOptions.length > 0){
                    for (var i = 0; i < xiangmuCeshiTypesOptions.length; i++) {
                            xiangmuCeshiTypesSelectSearch.add(new Option(xiangmuCeshiTypesOptions[i].indexName,xiangmuCeshiTypesOptions[i].codeIndex));
                    }
                }
            }
        }
    ...
    //查询当前表搜索条件所有列表
            function xiangmuCeshiTypesSelect() {
                //填充下拉框选项
                http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xiangmu_ceshi_types", "GET", {}, (res) => {
                    if(res.code == 0){
                        xiangmuCeshiTypesOptions = res.data.list;
                    }
                });
            }
