            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑测试</li>
                    </ol>
                </div>
            </div>
        </div>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="xiangmuId" name="xiangmuId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>模块名称</label>
                                        <input id="xiangmuCeshiName" name="xiangmuCeshiName" class="form-control"
                                               placeholder="模块名称">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>模块状态</label>
                                        <select id="xiangmuCeshiTypesSelect" name="xiangmuCeshiTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>测试详情</label>
                                        <input id="xiangmuCeshiContentupload" name="file" type="file">
                                        <script id="xiangmuCeshiContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var xiangmuCeshiContentUe = UE.getEditor('xiangmuCeshiContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="xiangmuCeshiContent" id="xiangmuCeshiContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#xiangmuCeshiContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('xiangmuCeshiContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("xiangmuCeshi/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addxiangmuCeshi');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationXiangmuceshitypesSelect(){
            var xiangmuCeshiTypesSelect = document.getElementById('xiangmuCeshiTypesSelect');
            if(xiangmuCeshiTypesSelect != null && xiangmuCeshiTypesOptions != null  && xiangmuCeshiTypesOptions.length > 0 ){
                for (var i = 0; i < xiangmuCeshiTypesOptions.length; i++) {
                    xiangmuCeshiTypesSelect.add(new Option(xiangmuCeshiTypesOptions[i].indexName,xiangmuCeshiTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(xiangmuCeshiTypesSelect != null && xiangmuCeshiTypesOptions != null  && xiangmuCeshiTypesOptions.length > 0 ) {
            for (var i = 0; i < xiangmuCeshiTypesOptions.length; i++) {
                if (xiangmuCeshiTypesOptions[i].codeIndex == ruleForm.xiangmuCeshiTypes) {//下拉框value对比,如果一致就赋值汉字
                        xiangmuCeshiTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.xiangmuCeshiContent != null && ruleForm.xiangmuCeshiContent != 'null' && ruleForm.xiangmuCeshiContent != '' && $("#xiangmuCeshiContentupload").length>0) {

            var xiangmuCeshiContentUeditor = UE.getEditor('xiangmuCeshiContentEditor');
            xiangmuCeshiContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.xiangmuCeshiContent != null){
                    mes = ''+ ruleForm.xiangmuCeshiContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                xiangmuCeshiContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#xiangmuCeshiContentupload").length>0) {
            var xiangmuCeshiContentEditor = UE.getEditor('xiangmuCeshiContentEditor');
            if (xiangmuCeshiContentEditor.hasContents()) {
                $('#xiangmuCeshiContent-input').attr('value', xiangmuCeshiContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ceshiId: "required",
                xiangmuId: "required",
                xiangmuCeshiName: "required",
                xiangmuCeshiTypes: "required",
                xiangmuCeshiContent: "required",
                insertTime: "required",
            },
            messages: {
                ceshiId: "测试人员不能为空",
                xiangmuId: "项目不能为空",
                xiangmuCeshiName: "模块名称不能为空",
                xiangmuCeshiTypes: "模块状态不能为空",
                xiangmuCeshiContent: "测试详情不能为空",
                insertTime: "添加时间不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addxiangmuCeshi = window.sessionStorage.getItem("addxiangmuCeshi");
        if (addxiangmuCeshi != null && addxiangmuCeshi != "" && addxiangmuCeshi != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("xiangmuCeshi/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#xiangmuCeshiName").val(ruleForm.xiangmuCeshiName);
        $("#xiangmuCeshiContent").val(ruleForm.xiangmuCeshiContent);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationXiangmuceshitypesSelect();
            <!--  初始化级联表的下拉框  -->
